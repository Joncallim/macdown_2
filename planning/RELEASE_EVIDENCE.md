...
| E19 math | in progress — Slices 1-3 of 4 done on branch `epic-19-math` (span scanner/contribution, Export rendering, Preview integration); not yet merged. See `planning/epic-19-implementation.md`'s as-built notes for full detail | package `swift test --no-parallel`: 1172/1172 across 130 suites; app-target `MacDown2Tests`: 132/132 serially (three pre-existing, unrelated `ExternalFileController*`/`WindowCoordinatorSaveAs*` tests are flaky only under parallel execution, confirmed unrelated); a 12-case shared `MathParityCorpus` exercised identically by both Preview's and Export's validity checks | executed against the real Release build (launched and driven live, not build-only): inline/display math renders correctly (including after a fix for multi-line `$$...$$` blocks, found by this dogfood pass itself — see below); malformed math shows the visible marker; partially-typed equations stay inert; escaped `$` stays literal; the documented `$5, $10` grammar ambiguity behaves as designed; dark/light theme legibility holds; math coexists with bold/italic/code/links; math correctly stays literal inside a fenced code block; Debug and Release builds both succeed | a real Preview/Export divergence was found and fixed during dogfood (multi-line display math rendered as literal text in Preview only, root-caused to Textual's own per-run tokenizer, fixed by collapsing internal newlines pre-Textual) — the kind of defect only live dogfood, not the automated corpus alone, could surface | none known at P0/P1; two documented, accepted residual risks: (1) no structured parse-error message for malformed math (`SwiftUIMath` exposes none); (2) Preview accessibility labelling for a rendered equation is confirmed NOT attachable via any public/observed Textual API (checked directly against Textual's `Attachment` protocol source, not assumed) — Export's `alt` text already covers the HTML/PDF path | **unverified — not yet merged**: Slice 4 (final accessibility/reconciliation pass), a large-equation-document Release performance measurement, the full adversarial corpus, and the final adversarial implementation review are still open before this can be called `passed` |
