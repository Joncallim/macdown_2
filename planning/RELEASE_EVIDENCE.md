...
| E19 math | Core feature (Slices 1-4) merged to `master` as PR #61 (`306e291`), including a completed post-implementation adversarial review with all findings fixed before merge. After merge, this repo's automated review bot (`chatgpt-codex-connector`) found three further real P1 defects on the merged commit — all reproduced and fixed on follow-up branch `epic-19-codex-fixes` (`planning/epic-19-implementation.md` §20), not yet merged | package `swift test --no-parallel`: 1183/1183 across 130 suites (was 1179 pre-fix); app-target `MacDown2Tests`: 138/138 serially (was 137; three pre-existing, unrelated `ExternalFileController*`/`WindowCoordinatorSaveAs*` tests are flaky only under parallel execution, confirmed unrelated); a 12-case shared `MathParityCorpus` exercised identically by both Preview's and Export's validity checks; every post-merge fix has a new test exercising the REAL `ParseEngine`/`ExportService`/cmark pipeline, not just the affected function in isolation | executed against the real Release build across two dogfood passes before merge (inline/display rendering, malformed-equation marker, partially-typed-equation safety, escaped-`$` literalness, the `$5, $10` grammar ambiguity, dark/light legibility, code-fence protection); the post-merge fix pass could NOT be re-verified visually — the environment's screen-capture/window-accessibility access became unreliable during that pass (`app_screenshot` failed with a capture-stream error; `System Events` stopped enumerating the app's windows despite seeing the process) on a machine where both had worked earlier in this same epic — recorded honestly as an exhausted tooling blocker, not skipped silently; the fixes are instead backed by real (non-mocked, non-unit-isolated) `ExportService`/cmark pipeline tests, which is what actually produces the HTML a browser renders | pre-merge: two real defects found and fixed by the pre-merge adversarial review (a code-fence content-corruption gap; a PDF dark-theme contrast bug), one Preview/Export divergence found and fixed by dogfood (multi-line display math). Post-merge: three further real defects found by the automated review bot and fixed — (1) an escaped `\$` immediately before real math absorbed the real equation's opening delimiter, losing it entirely; (2) inline code containing math-like text was treated as math, and cmark's sentinel-substitution mechanism does not run inside a CODE node, so the exported document showed a literal garbage sentinel string instead of the author's code; (3) exported equations rendered roughly 3× too large because the `<img>` tag carried no `width`/`height` DPI hint for its 3×-scaled PNG | none known at P0/P1; four documented, accepted residual risks: (1) no structured parse-error message for malformed math; (2) Preview accessibility labelling for a rendered equation is confirmed NOT attachable via any public Textual API; (3) a code fence nested inside a list item or block quote (not top-level) is not covered by the code-fence exclusion fix; (4) Preview inherits Textual's own escaped-dollar bug (finding 1 above) since Export can be fixed independently but Preview's rendering depends on Textual's own sealed tokenizer | **unverified — the Codex-fix branch is not yet merged**: a large-equation-document Release performance measurement, the remaining narrower adversarial-corpus cases, and a live visual re-check of the post-merge fixes (blocked by the tooling issue above, to retry when the environment's screen capture is healthy) remain open before this can be called `passed` |
