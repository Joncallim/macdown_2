...
- **Slice 3a** — Foundation: `EditorTextSystem.selectionSet`/computed `selectedRange`, `EditorView.Coordinator`'s selection-set-aware `textViewDidChangeSelection`, Escape-collapse, and multi-range typing/delete/paste/indent/comment-toggle routed through `EditorEditTransaction` per §7.2 — the piece every later part of Slice 3 (and Slice 4's multi-cursor-aware transforms, per its own stated dependency on `EditorEditTransaction`) needs to exist first.
...
- **Slice 3a** — Foundation: `selectionSet`/computed `selectedRange`, selection-set-aware selection-change handling, Escape-collapse, multi-range typing/delete/paste/indent/comment-toggle via `EditorEditTransaction` (§6.9).
