...
- **No recursive whole-tree traversal exists anywhere** — `FileTree`'s `DirectoryReading`/`FileSystemDirectoryReader` is single-level, lazy, expand-on-click only; the sidebar never walks more than the currently-expanded set. `WorkspaceFileIndex`'s recursive walk is genuinely new code, though it should call the same `DirectoryReading.contents(of:)` primitive per directory (for consistent hidden/package/symlink classification) and must add its own symlink-loop guard (nothing today prevents infinite recursion in a *recursive* walk — `FileTreeCopySafety`'s ancestor check solves a different problem, move/copy destination validation, not traversal).
...
public struct EditorEditTransaction: Sendable {
    /// Must be non-overlapping; validated at construction (fatal in Debug,
    /// silently drops overlapping replacements in Release rather than
    /// corrupting text — mirrors the existing "fail open" discipline).
...
**Ownership:** `EditorCore` + new `TextSearch` package only. No changes to `EditorView`, `EditorTextSystem.apply(_:)`'s actual behavior, `DocumentEditorSplitView`, or any app-target file.
...
**Stop condition:** if any of these types cannot be built without touching `EditorTextSystem`'s existing public API in a way not anticipated by §6, stop and reconcile this document before proceeding — do not silently widen scope into Slice 2's territory.
