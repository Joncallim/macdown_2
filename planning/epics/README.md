...
`formats/export/settings → E14 contribution infrastructure → E19 math → E20 diagram platform + Mermaid → E21 renderer admission/integration → feature-complete gate → E15 whole-app polish/first-run → E16 translation infrastructure/current baseline → E22 editor essentials → public identity re-freeze → E23 Mac integration & personalisation → closed-epic debt gate (#115) → final E16 localisation/string freeze → E17 migration/distribution`
...
**2026-09-21 controlled reopening:** after the original feature-complete gate and most E16 work, the owner explicitly approved two bounded pre-release epics. E22 closes high-frequency general-text workflows; E23 closes themes, Markdown Quick Look and Finder integration and is the **last planned macOS 1.0 capability epic**. Because these epics add UI/strings, E16's final string freeze occurs after them. Because the product will ship as a new editor descended from MacDown rather than as `MacDown 2`, the prior public-identity freeze is also reopened and must be re-frozen before E23 commits final bundle/extension/theme-file identifiers.
...
| E11 | Multi-format: JSON, HTML, LaTeX source + language registry | M4 — Workspace & formats | E05, E07 | ✅ done (issue #12; JSON tooling, HTML preview toggle, language registry completion. LaTeX/TeX source recognition from the amended scope was **not** implemented in this merge; the 2026-09-21 closed-epic audit (#115) assigns it as mandatory pre-1.0 inherited work in E22 (#112)) |
...
| E13 | Settings | M4 — Workspace & formats | E02 | ✅ done (issue #14; PR #52. Legacy-key migration map/full O3 import tracked by #53; #115 now requires #53 to close before macOS 1.0, with parser-model cleanup in E22 and actual legacy import in E17) |
...
| E21 | Evaluate/integrate D2, Graphviz/DOT, WaveDrom | M5 — feature completion | E20 | ✅ done (issue #46, closed 2026-09-18; PRs #81-#85). D2 and Graphviz ACCEPTED, wired through Export + native Preview, adversarially hardened, and now have genuinely executed UI-test evidence (a first for this project's diagram epics — see `RELEASE_EVIDENCE.md`). WaveDrom was DEFERRED at E21 closure; the 2026-09-21 closed-epic audit (#115) finalised it as **REJECTED FOR macOS 1.0** over the same disclosed `eval()`/CVE-2026-50733 finding (amendment recorded on #46). Two real, previously-undisclosed gaps found while closing this issue: no interactive copy-as-SVG affordance for any diagram language ([#86](https://github.com/Joncallim/macdown_2/issues/86)), and most of `MacDown2UITests` fails when actually executed, confirmed pre-existing ([#88](https://github.com/Joncallim/macdown_2/issues/88)) |
| Gate | macOS 1.0 feature-complete audit | before E15 | all major feature epics through E21 | ✅ passed 2026-09-19; no P0/P1 found and UI-test evidence advanced (15/30 → 3/30 failing, PR #90). The then-frozen `MacDown 2` identity was explicitly reopened on 2026-09-21 after the owner chose to ship a new product descended from MacDown; final identity re-freeze now occurs before E23 final identifiers and the final E16 freeze. |
| E15 | Liquid Glass + accessibility + whole-app audit + final first-run | M5 — polish & ship | frozen identity, E09, E19, E21 + feature-complete gate | ✅ accepted (2026-09-20) — issue #16, two explicit release prerequisites carried forward (final icon, final live VoiceOver/visual pass), not release-blocking for E16/E17. Real progress: #86 (copy-as-SVG) resolved, #70 (reload banner) resolved as a side effect of the #88 accessibility-identifier fix, #63 (nested-fence math corruption) fixed, E12/E19 inherited evidence debt closed (real PDF-pagination test, real math-rendering performance measurement), icon-only-button VoiceOver labels added, UI-test reliability substantially advanced (#88: 15/30 → 3/30 failing, precise root causes for the rest), text round-trip fidelity corpus executed (12 real file-write/read-back tests, no unexpected normalization found), and the app-had-no-icon gap closed with a functional placeholder (PR #99 — a code-generated icon at all 10 required sizes; explicitly not final design work, real artwork remains deliberately deferred). A live visual/accessibility dogfood pass was attempted again this session via `computer-use` (a different mechanism than prior attempts, per explicit owner direction): the app itself became grantable/enumerable for the first time, but window screen-capture failed outright (`audio/video capture failure`) and System Settings/VoiceOver Utility access was declined, so no screenshot or VoiceOver/Reduce Transparency/Increase Contrast/Reduce Motion evidence was obtained — recorded honestly in `RELEASE_EVIDENCE.md` rather than assumed. The whole-app 20-tab performance gap closed too (PR #101): a real, non-mocked test at 20-tab scale measured materializing 20 tabs at ~267 ms and re-visiting all 20 already-open tabs (a real tab switch) at ~0.02 ms, confirming the per-tab cache design genuinely delivers cheap switching rather than assuming it; landing it also surfaced and fixed a real cross-suite resource leak (20 un-released `EditorTextSystem`/highlighter graphs starving a different, pre-existing suite on CI). The first-run welcome screen closed too (PR #103): a first-launch-only window offering "Start Writing" or "Open a Sample Document" (a real tour of formatting/code/math/diagrams, seeded through the same document-update API a live edit uses), gated by a single preference bool rather than a new `AppSettings` domain field to avoid that domain's Codable-migration risk. Per explicit owner direction (2026-09-20), the two remaining blocked items now carry a settled disposition rather than pausing the mission: **the placeholder icon is a declared external release-asset dependency** ("⚠️ FINAL RELEASE ASSET REQUIRED" in `RELEASE_EVIDENCE.md`) — not a P0/P1, not a blocker for E16/E17, verified this pass that the asset catalog/build pipeline are correctly wired for a drop-in final icon (all 10 slots present, Release build genuinely consumes the catalog with zero `actool` warnings) — and **live screen-capture/VoiceOver attempts have stopped** (sufficiently demonstrated blocked) in favor of the strongest deterministic evidence obtainable without them: real render evidence for 15 app states via `ImageRenderer` (PR #105, CI-executed and passed, plus 5 outputs personally visually inspected via a standalone executable), a real `AXUIElement` menu-bar accessibility-tree walk (2373 elements, confirmed working via a permission distinct from screen capture, though per-window content AX is blocked by the same no-WindowServer-session root cause, independently reconfirmed via `NSRunningApplication.activate()` returning `false`), and a source-level accessibility audit (zero new defects found). Live VoiceOver and `MacDown2UITests`' real execution remain explicit, carried-forward manual/CI-environment prerequisites, not silently passed. The Liquid Glass/visual coherence code audit completed too: zero custom backgrounds behind system bars, zero glass-on-glass stacking (exactly 3 `Material` usages app-wide, each a single non-stacked overlay), sidebar/split-view fully native. E15 closure reconciliation then re-ran and reconciled the whole verification matrix (master's post-merge CI: package + full app-target test suites, lint/static checks, Debug/Release/CLI builds) — all green — and confirmed zero open P0/P1 issues. Closed historically with two carried-forward prerequisites (final icon, final live VoiceOver/visual pass). The 2026-09-21 audit (#115) promotes both to mandatory pre-production-RC closure: icon in E23, live accessibility/visual proof in #115. |
| E16 | Localization + in-app string freeze | M5 — polish & ship | final identity + final E15 in-app UI/copy | 🔶 interim baseline accepted (2026-09-21), final freeze still pending E22/E23/#115 — issue #17. PR #108 + PR #110 (merged): String Catalog infrastructure added to the app target and, after a grep-based audit found 8 of 9 `MacDownKit` package targets had no resource bundle at all (silently disconnecting every `String(localized:)` call inside them from any catalog), to all 9 package targets too. `xcodebuild -exportLocalizations` audits fixed the app target's extraction bypasses 32 → 11 → 0 across those two PRs (each count independently re-verified, not just claimed). **This pass**: found and fixed a third, previously-undiscovered app-target gap — 15 `String(localized:)` literals in Preview's contribution-admission pipeline (`PreviewContributionAdmission`/`Adapter`/`Session.swift`) that never reached the catalog — plus a real functional bug found while investigating: the app's one pluralized string used `String(localized:)` for Foundation's `^[...](inflect: true)` grammar-agreement markup, which that API does not interpret (only `AttributedString`/`Text` do), so the raw markup was leaking into the Preview diagnostics panel in English; fixed by resolving through `AttributedString`. Added real (not pseudo) `fr`/`pl`/`ja` translations across all 10 catalogs (141 keys), chosen for three materially different CLDR plural-rule families, satisfying the epic's "≥3 materially different plural-rule languages" and "at least one shipped non-English locale" acceptance criteria; verified every catalog compiles cleanly for those locales via `xcrun xcstringstool compile` (including the one hand-authored plural-variation entry, which has no other scriptable validation path outside Xcode's own String Catalog editor). Added a real `.tx/config` (Transifex CLI, confirmed `XCSTRINGS` file type support) and documented push/pull workflow (`planning/epic-16-implementation.md`) — org/project identity is an honest placeholder pending the release owner's Transifex account, the one genuine external dependency in this epic. Added `PseudoLocalizationLayoutTests.swift` (16D): real production views rendered under fr/pl/ja via `.environment(\.locale:)`, plus synthetic-worst-case-length structural tests and a locale-parameterized plural-resolution test. Reviewed the epic's terminology list against the now-complete ~200-string English surface (16E) — already consistent, no changes needed. **Verified this pass**: package `swift build` clean, full package suite 1296/1296 passed, app Debug build succeeds, `swiftformat`/`swiftlint --strict` 0 violations/531 files. The app-target `MacDown2Tests` suite could not run locally (`xcodebuild test` hung at "Resolve Package Graph", a previously-documented local-only limitation) — hosted CI ran it instead. CI's first run correctly caught two real issues a local pass couldn't (a post-verification `swiftformat` violation, and a genuine test-design bug: `shortcutHintHandlesVeryLongLabel` rendered on a transparent `ImageRenderer` canvas where premultiplied-alpha black text and the transparent background share identical RGB channels, so the RGB-only blank-detector never saw the text — fixed with an explicit opaque background). After both fixes, CI passed in full, including all locale-parameterized production-view renders (fr/pl/ja) and the plural-resolution test across en/fr/pl/ja. **PR #111 merged 2026-09-21 (commit `eca77d3`).** The then-planned 16F freeze was superseded by the 2026-09-21 controlled reopening: this catalog work is accepted as the current interim localisation baseline, not the 1.0 string freeze; the final freeze occurs only after E22/E23 and #115, per the current sequencing above. Carried-forward pre-freeze items: native-speaker QA of the seeded fr/pl/ja text; personal visual inspection of the localized renders for subtle (non-crash) clipping, not yet obtainable locally; creation of the real Transifex organization/project (the `.tx/config` workflow itself is real and usable today). |
| E22 | Editor essentials: power-text workflows before macOS 1.0 | M5 — pre-release completion | E15 + E16 current localisation baseline | 🔶 open — issue [#112](https://github.com/Joncallim/macdown_2/issues/112). Adds the bounded general-text essentials required for credible day-to-day use; it deliberately reopens the affected E15/E16 evidence rather than pretending the prior string freeze still applies. |
| E23 | Mac integration & personalisation: themes, Quick Look, Finder polish | M5 — pre-release completion | E22 + final public identity | 🔶 open — issue [#113](https://github.com/Joncallim/macdown_2/issues/113). Final planned macOS 1.0 capability epic: semantic/bundled/custom themes, a safe Markdown Quick Look extension, and Finder/document-association polish. |
| Gate | Closed-epic carried-forward debt closure | before final E16 freeze / E17 production RC | E22, E23 + all historical release-evidence debt | 🔶 open — issue [#115](https://github.com/Joncallim/macdown_2/issues/115). Every carried-forward item from a closed epic must end FIXED, PROVED, or (only where the original candidate contract allowed it) REJECTED FOR 1.0. Environment-blocked/manual-later/accepted-residual-debt is not a production-release disposition. |
| E17 | Stateful migration, distribution & release | M5 — polish & ship | E22/E23 + #115 debt gate + final E16 freeze | open — issue #18; production RC/publish is blocked until #115 closes. |
...
E19 + E21 + remaining original Mac features
  ...
  ─▶ E15 (whole-app/first-run polish)
