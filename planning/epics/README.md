...
| E15 | Liquid Glass + accessibility + whole-app audit + final first-run | M5 — polish & ship | frozen identity, E09, E19, E21 + feature-complete gate | 🔶 in progress — issue #16. Real progress: #86 (copy-as-SVG) resolved, #70 (reload banner) resolved as a side effect of the #88 accessibility-identifier fix, #63 (nested-fence math corruption) fixed, E12/E19 inherited evidence debt closed (real PDF-pagination test, real math-rendering performance measurement), icon-only-button VoiceOver labels added, UI-test reliability substantially advanced (#88: 15/30 → 3/30 failing, precise root causes for the rest). Not yet started: text round-trip fidelity corpus, Liquid Glass/visual coherence audit, final app icon/marketing assets, first-run/onboarding UI, whole-app 20-tab performance/memory report |
