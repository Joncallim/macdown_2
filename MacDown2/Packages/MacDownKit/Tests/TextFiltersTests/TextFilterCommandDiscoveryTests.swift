...
struct TextFilterCommandDiscoveryTests {
    ...
    // MARK: - Post-review finding #15: symlinks are not "regular files"

    @Test func rejectsAnExecutableSymlinkToARegularFile() throws {
        let directory = try TextFilterFixtures.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let real = try TextFilterFixtures.makeExecutableScript(
            TextFilterFixtures.echoStdin, named: "real.sh", in: directory
        )
        let link = directory.appendingPathComponent("link.sh")
        try FileManager.default.createSymbolicLink(at: link, withDestinationURL: real)

        let commands = TextFilterCommandDiscovery.discoverCommands(in: directory)

        #expect(commands.map(\.id) == ["real.sh"])
    }

    @Test func rejectsASymlinkToADirectory() throws {
        let directory = try TextFilterFixtures.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let nested = directory.appendingPathComponent("nested", isDirectory: true)
        try FileManager.default.createDirectory(at: nested, withIntermediateDirectories: true)
        let link = directory.appendingPathComponent("linkToDirectory")
        try FileManager.default.createSymbolicLink(at: link, withDestinationURL: nested)

        #expect(TextFilterCommandDiscovery.discoverCommands(in: directory).isEmpty)
    }

    @Test func rejectsABrokenSymlink() throws {
        let directory = try TextFilterFixtures.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let link = directory.appendingPathComponent("broken.sh")
        try FileManager.default.createSymbolicLink(
            at: link, withDestinationURL: directory.appendingPathComponent("gone.sh")
        )

        #expect(TextFilterCommandDiscovery.discoverCommands(in: directory).isEmpty)
    }

    // MARK: - Post-review finding #11: colliding display names are disambiguated

    @Test func disambiguatesCollidingHumanizedNamesWithTheRealFilename() throws {
        let directory = try TextFilterFixtures.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        try TextFilterFixtures.makeExecutableScript(TextFilterFixtures.echoStdin, named: "foo.sh", in: directory)
        try TextFilterFixtures.makeExecutableScript(TextFilterFixtures.echoStdin, named: "foo.py", in: directory)

        let commands = TextFilterCommandDiscovery.discoverCommands(in: directory)

        #expect(Set(commands.map(\.name)).count == 2)
        for command in commands {
            #expect(command.name.contains(command.id))
        }
    }

    @Test func nonCollidingNamesAreUnaffectedByDisambiguation() throws {
        let directory = try TextFilterFixtures.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        try TextFilterFixtures.makeExecutableScript(TextFilterFixtures.echoStdin, named: "unique.sh", in: directory)

        let commands = TextFilterCommandDiscovery.discoverCommands(in: directory)

        #expect(commands.first?.name == "Unique")
    }

    @Test func sortOrderIsDeterministicWhenDisplayNamesTie() throws {
        let directory = try TextFilterFixtures.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        try TextFilterFixtures.makeExecutableScript(TextFilterFixtures.echoStdin, named: "foo.sh", in: directory)
        try TextFilterFixtures.makeExecutableScript(TextFilterFixtures.echoStdin, named: "foo.py", in: directory)

        let first = TextFilterCommandDiscovery.discoverCommands(in: directory).map(\.id)
        let second = TextFilterCommandDiscovery.discoverCommands(in: directory).map(\.id)

        #expect(first == ["foo.py", "foo.sh"]) // "foo.py" < "foo.sh" lexicographically
        #expect(first == second)
    }
}
