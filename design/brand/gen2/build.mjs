...
const m = (k, cls = '', fill = 'currentColor') =>
  `<svg class="${cls}" viewBox="0 0 100 100" aria-hidden="true" style="color:${fill}">${marks[k].svg}</svg>`;
...
const icon = (k, v, cls) => `<svg class="${cls}" viewBox="0 0 100 100" role="img" aria-label="${marks[k].name} ${v} app icon">
  ...
  <g transform="translate(21 21) scale(.58)" style="color:url(#fg-${v}-${k}-${cls})">${marks[k].svg.replaceAll('currentColor', `url(#fg-${v}-${k}-${cls})`).replaceAll('id="lc"', `id="lc-${v}-${cls}"`).replaceAll('url(#lc)', `url(#lc-${v}-${cls})`)}</g></svg>`;
...
writeFileSync('board.html', tpl
  ...
  .replace('{{TABLE}}', table)
  .replace('{{CROWN}}', icon('crown','light','fin')).replace('{{LIGATURE}}', icon('ligature','light','fin'))
  .replace('{{MARKS}}', JSON.stringify(Object.fromEntries(Object.keys(marks).map(k=>[k,marks[k].svg])))));
for (const k of Object.keys(marks)) writeFileSync(`${k}.svg`, `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" style="color:#000">${marks[k].svg}</svg>\n`);
console.log('built');
